...
import AdvancedLeadResearchService, { AdvancedLeadResearchParams } from '../services/advanced-lead-research.js';

const router = express.Router();
const prisma = new PrismaClient();

// Send message to copilot
router.post('/chat/:campaignId', authenticateToken, async (req: any, res) => {
  try {
    ...
    const aiResponse = await OpenAIService.generateCopilotResponse(
      message,
      conversationHistory || [],
      user,
      campaign
...
router.post('/research/:campaignId', authenticateToken, async (req: any, res) => {
  try {
    const { campaignId } = req.params;
...
async function conductLeadResearch(campaignId: string, campaign: any, userId: string) {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId }
    });
    ...
    const researched_leads = await AdvancedLeadResearchService.conductAdvancedResearch({
      campaignId,
      icpDetails,
      userContext: user,
      targetCount: 50,
    ...
    await AdvancedLeadResearchService.saveAdvancedLeadsToDatabase(campaignId, researched_leads);
